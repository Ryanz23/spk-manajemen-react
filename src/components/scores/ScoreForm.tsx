...
import { updateAlternativeScores } from '@api/alternatives';
...
interface Props {
  ...
  onFetchExistingScores?: (criteriaIds: string[], alternativeIds: string[]) => Promise<Record<string, Record<string, number>>>;
}
...
const ScoreForm: React.FC<Props> = ({ 
  ...
  const [allCriteria, setAllCriteria] = useState<Criterion[]>(availableCriteria);
  const [allAlternatives, setAllAlternatives] = useState<Alternative[]>(availableAlternatives);
  ...
  const [existingScores, setExistingScores] = useState<Record<string, Record<string, number>>>({});
  ...
  useEffect(() => {
    ...
    fetchInitialData();
  }, [onFetchCriteria, onFetchAlternatives, allCriteria.length, allAlternatives.length]);
  ...
  useEffect(() => {
    const fetchScores = async () => {
      if (selectedCriteriaIds.length > 0 && selectedAlternativeIds.length > 0 && onFetchExistingScores) {
        try {
          const existingData = await onFetchExistingScores(selectedCriteriaIds, selectedAlternativeIds);
    ...
    if (selectedAlternatives.length > 0 && selectedCriteria.length > 0) {
      ...
      selectedAlternatives.forEach(alt => {
        ...
        selectedCriteria.forEach(crit => {
          initialScores[alt.id][crit.id] = existingScores[alt.id]?.[crit.id] || 0;
        });
      });
  ...
  const handleAlternativeSelection = (alternativeId: string, checked: boolean) => {
  ...
  const handleScoreChange = (alternativeId: string, criteriaId: string, value: number) => {
    setScores(prev => ({
      ...prev,
      [alternativeId]: {
        ...prev[alternativeId],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updatePromises = selectedAlternativeIds.map(async (alternativeId) => {
        console.log('Updating scores for alternative:', alternativeId, 'with scores:', scores[alternativeId]);

        try {
          await updateAlternativeScores(alternativeId, scores[alternativeId]);
        } catch (err) {
          console.error(`Failed to update scores for alternative ${alternativeId}:`, err);
          throw err;
        }
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
          ...
          {allCriteria.length === 0 ? (
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {allCriteria.map(criterion => (
                <label key={criterion.id} className="flex items-center space-x-2">
                  <input
                    type="checkbox"
                    checked={selectedCriteriaIds.includes(criterion.id)}
                    onChange={(e) => handleCriteriaSelection(criterion.id, e.target.checked)}
                    className="rounded"
                  />
        ...
        <div className="border border-gray-300 rounded p-4">
          <h3 className="font-semibold mb-3 text-gray-700">Pilih Alternatif:</h3>
          {allAlternatives.length === 0 ? (
            <p className="text-gray-500">Tidak ada alternatif tersedia</p>
          ) : (
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {allAlternatives.map(alternative => (
                <label key={alternative.id} className="flex items-center space-x-2">
                  <input
                    type="checkbox"
                    checked={selectedAlternativeIds.includes(alternative.id)}
                    onChange={(e) => handleAlternativeSelection(alternative.id, e.target.checked)}
                    className="rounded"
                  />
                ...
                <tbody>
                  {selectedAlternatives.map(alternative => (
                    <tr key={alternative.id}>
                      ...
                      {selectedCriteria.map(criterion => (
                        <td key={criterion.id} className="border border-gray-300 px-2 py-2">
                          <input
                            type="number"
                            min="0"
                            max="100"
        ...
        <div className="border-t pt-6">
          <div className="p-4 bg-gray-100 border border-gray-300 rounded text-center">
            <p className="text-gray-600">
              Pilih minimal satu kriteria dan satu alternatif untuk mulai input skor
            </p>
          </div>
        </div>
      )}
